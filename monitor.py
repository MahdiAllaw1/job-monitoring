...
MAX_BROWSER_CLICKS = int(os.environ.get("MAX_BROWSER_CLICKS", "18"))
BROWSER_TIMEOUT_MS = int(os.environ.get("BROWSER_TIMEOUT_MS", "60000"))
...
JOB_HREF_PATTERNS = [
    "/job/", "/jobs/", "/careers/", "/career/", "/details/", "/offres-demploi/", "/offre", "/emploi",
    "jobid=", "job_id=", "jobId=", "reqid=", "requisition", "position", "posting",
    "search/jobdetails", "vacancies", "openings", "lever.co", "greenhouse.io", "jobs.jobvite.com",
]

BAD_TEXT = {
    "home", "homepage", "privacy", "cookies", "terms", "contact", "linkedin", "facebook",
    "youtube", "instagram", "twitter", "x", "search", "clear", "reset", "read more",
    "more", "voir plus", "show more", "load more", "show jobs", "show job", "afficher plus",
    ...
    "chercher sur le site global", "careers", "carrières", "travailler chez apple", "présentation",
    "parcours chez apple", "france", "english", "français",
}

BAD_TITLE_RE = re.compile(
    r"^(home|careers?|carri[eè]res|search|show\s+\d*\s*more|afficher|voir|read more|more|apply|postuler|france|english|fran[cç]ais)$",
    re.I,
)
...
BAD_HREF_PARTS = [
    ...
    "/jobs/show_more", "/jobs/show-more", "choose-country-region", "/careers/fr/index", "work-at-apple",
]
...
CLICK_BUTTON_RE = re.compile(
    r"(show\s*(?:\d+\s*)?more|load\s*more|more\s*jobs|show\s*more\s*requisitions|show\s+\d+\s+more|voir\s*plus|afficher\s*plus|charger\s*plus|plus\s*d.offres|voir\s*les\s*offres\s*suivantes)",
    re.I,
)
...
def stable_id(company: str, title: str, url: str) -> str:
    ...
    for key in list(cleaned_query):
        k = key.lower()
        if k.startswith(("utm_", "gh_src")) or k in {"src", "source", "ref", "triggergobutton", "iis", "iisn"}:
...
def clean_title(text: str) -> str:
    ...
    text = re.sub(r"\s*Partager\s+.*$", "", text, flags=re.I)
    text = re.sub(r"\s+Num[eé]ro de r[oô]le\s*:.*$", "", text, flags=re.I)
    text = normalize_space(text)
    if len(text) > 180:
...
def is_bad_href(href: str) -> bool:
    ...
    return any(low_href.startswith(bad) or bad in low_href for bad in BAD_HREF_PARTS)


def is_bad_title(title: str) -> bool:
    low = normalize_space(title).lower()
    return not low or len(low) < 3 or low in BAD_TEXT or bool(BAD_TITLE_RE.search(low))
...
def looks_like_job_link(href: str, text: str) -> bool:
    ...
    if is_bad_href(low_href):
        return False
    if is_bad_title(low_text):
        return False
    if len(low_text) > 220:
        return any(x in low_href for x in ["/job/", "/jobs/", "/details/", "jobid", "requisition", "posting"])
...
def text_until_marker(raw_text: str, start_marker: str, end_markers: Iterable[str]) -> str:
    low = raw_text.lower()
    ...
    for marker in end_markers:
        idx = low.find(marker.lower(), start + len(start_marker))
        if idx != -1:
            cut = min(cut, idx)
    return raw_text[start:cut]


def parent_context(a: Tag, max_depth: int = 5, max_len: int = 1800) -> str:
    context = normalize_space(a.get_text(" ", strip=True))
    n = a
    for _ in range(max_depth):
        if not isinstance(n, Tag) or n.parent is None:
            break
        n = n.parent
        txt = normalize_space(n.get_text(" ", strip=True))
        if len(context) < len(txt) <= max_len:
            context = txt
    return context
...
def extract_location_from_context(context: str) -> str:
    ...
    # Apple-style: LieuFrance : plusieurs lieux
    m = re.search(r"Lieu\s*([^:]{0,20}:?\s*[^A]{3,120}?)(?:Actions|Num[eé]ro|Heures|$)", context, re.I)
    if m:
        return normalize_space(m.group(1))[:120]
    patterns = [
        r"((?:[A-ZÀ-ÿ][\wÀ-ÿ'\-]+(?:\s+[A-ZÀ-ÿ][\wÀ-ÿ'\-]+){0,3}),\s*(?:France|FR))",
        r"((?:Paris|Grenoble|Meylan|Sophia[- ]Antipolis|Toulouse|Rousset|Aix-en-Provence|Courbevoie|La Ciotat|Gémenos|Nantes|Rennes|Nice|Lyon|Marseille|Crolles|Caen|Pessac|Valbonne|Biot|Montigny-le-Bretonneux|Bussy-Saint-Georges|Douai|Osny|Vélizy-Villacoublay|Vitré)[^\n,;]{0,60})",
        r"((?:France|FR)\s*[:·-]\s*(?:plusieurs lieux|multiple locations|[A-ZÀ-ÿ][\wÀ-ÿ'\- ]+))",
    ]
    for pat in patterns:
...
def add_job(jobs: dict[str, Job], company_name: str, title: str, url: str, location: str = "", meta: str = "") -> None:
    ...
    jobs[jid] = Job(id=jid, company=company_name, title=title, url=url, location=normalize_space(location)[:160], meta=meta)
...
def jobs_from_soup(company_name: str, source_url: str, soup: BeautifulSoup, *, meta: str = "html") -> list[Job]:
    ...
    for a in soup.select("a[href]"):
        ...
        if not looks_like_job_link(href, text):
            continue
        context = parent_context(a)
        if any(marker in context.lower()[:250] for marker in SUGGESTION_MARKERS):
            continue
        add_job(jobs, company_name, text, url, extract_location_from_context(context), meta)
...
def parse_apple(company: dict[str, Any], source_url: str) -> list[Job]:
    r = fetch(source_url)
    ...
    for a in soup.select('a[href*="/details/"]'):
        ...
        url = urljoin(source_url, href)
        text = normalize_space(a.get_text(" ", strip=True) or a.get("aria-label", "") or a.get("title", ""))
        if is_bad_title(text) or text.lower().startswith(("voir ", "nos sites", "partager")):
            continue
        context = parent_context(a, max_depth=6, max_len=2500)
        role_id = ""
        m = re.search(r"Num[eé]ro de r[oô]le\s*:?\s*(\d+)", context, re.I)
        if m:
            role_id = m.group(1)
        # Stable by role ID when available, because Apple URLs can include localized slugs.
        jid_url = f"apple-role:{role_id}" if role_id else url
        add_job(jobs, company["name"], text, jid_url, extract_location_from_context(context), "apple-static")
        if role_id:
            # Put the real URL back after using role id for stable hash.
            jid = stable_id(company["name"], clean_title(text), jid_url)
            jobs[jid] = Job(id=jid, company=company["name"], title=clean_title(text), url=url, location=extract_location_from_context(context), meta="apple-static")
    max_results = int(company.get("apple_max_results", 0) or 0)
...
def parse_secure_ic(company: dict[str, Any], source_url: str) -> list[Job]:
    r = fetch(source_url)
    soup = BeautifulSoup(r.text, "html.parser")
    jobs: dict[str, Job] = {}
    for a in soup.select("a[href]"):
        ...
        context = parent_context(a, max_depth=4, max_len=1200)
        low_href = href.lower()
        low_context = context.lower()
        if "?types=" in low_href or "?locs=" in low_href or "open-positions" in low_href:
            continue
        if "read more" not in low_context and "read more" not in text.lower():
            continue
        title = re.sub(r"\bRead more\b", "", text, flags=re.I).strip()
        if not title or title.lower() in {"read more", "jobs"}:
            title = re.sub(r"\bRead more\b", "", context, flags=re.I).strip()
        title = re.sub(r"^(Commercial / Marketing|Recherche et d[eé]veloppement)\s*", "", title, flags=re.I)
        location = extract_location_from_context(context)
        add_job(jobs, company["name"], title, url, location, "secure-ic-static")
    return list(jobs.values())


def parse_icalps(company: dict[str, Any], source_url: str) -> list[Job]:
    r = fetch(source_url)
    soup = BeautifulSoup(r.text, "html.parser")
    jobs: dict[str, Job] = {}
    # Current IC'Alps page exposes the jobs as H3 links between Offres d'emploi and stages.
    for a in soup.select("h3 a[href]"):
        title = normalize_space(a.get_text(" ", strip=True))
        if any(x in title.lower() for x in ["candidature", "sealsq", "stage"]):
            continue
        url = urljoin(source_url, a.get("href", ""))
        add_job(jobs, company["name"], title, url, "Saint-Martin-d'Hères / Toulouse", "icalps-static")
    return list(jobs.values())


def parse_menta(company: dict[str, Any], source_url: str) -> list[Job]:
    r = fetch(source_url)
    soup = BeautifulSoup(r.text, "html.parser")
    jobs: dict[str, Job] = {}
    for a in soup.select("a[href]"):
        title = normalize_space(a.get_text(" ", strip=True))
        if is_bad_title(title) or title.lower() == "read more":
            continue
        context = parent_context(a, max_depth=4, max_len=1300)
        low = (title + " " + context).lower()
        if "north america" in low or "usa" in low:
            continue
        if "france" not in low and "sophia" not in low and "yerevan" not in low:
            continue
        # Avoid menu/footer links.
        job_words = ["engineer", "implementation", "verification", "software", "hardware", "director"]
        if not any(w in title.lower() for w in job_words):
            continue
        url = urljoin(source_url, a.get("href", ""))
        add_job(jobs, company["name"], title, url, extract_location_from_context(context), "menta-static")
    return list(jobs.values())


def parse_scalinx(company: dict[str, Any], source_url: str) -> list[Job]:
    r = fetch(source_url)
    soup = BeautifulSoup(r.text, "html.parser")
    jobs: dict[str, Job] = {}
    for heading in soup.find_all(["h2", "h3", "h4"]):
        title = normalize_space(heading.get_text(" ", strip=True))
        if is_bad_title(title) or "career" in title.lower():
            continue
        ...
        if "/jobs/" not in href.lower():
            continue
        url = urljoin(source_url, href)
        add_job(jobs, company["name"], title, url, "Paris", "scalinx-static")
    return list(jobs.values())


def parse_teamtailor(company: dict[str, Any], source_url: str) -> list[Job]:
    name = company["name"]
    jobs: dict[str, Job] = {}
    queue = [source_url]
    seen_pages: set[str] = set()
    max_pages = int(company.get("max_pages", 8))
    while queue and len(seen_pages) < max_pages:
        ...
        for a in soup.select('a[href*="/jobs/"]'):
            href = a.get("href", "")
            if "/jobs/show_more" in href.lower():
                show_url = urljoin(url, href)
                if show_url not in seen_pages and show_url not in queue:
                    queue.append(show_url)
                continue
            title = normalize_space(a.get_text(" ", strip=True))
            if is_bad_title(title):
                continue
            full_url = urljoin(url, href)
            context = parent_context(a, max_depth=5, max_len=1400)
            add_job(jobs, name, title, full_url, extract_location_from_context(context), "teamtailor")
        # Some Teamtailor pages expose the show-more URL as a regular link outside the /jobs selector.
        for a in soup.select('a[href*="show_more"]'):
            show_url = urljoin(url, a.get("href", ""))
            if show_url not in seen_pages and show_url not in queue:
                queue.append(show_url)
    return list(jobs.values())


def parse_jobvite(company: dict[str, Any], source_url: str) -> list[Job]:
    # Arteris embeds/redirects Jobvite. Jobvite sometimes renders only with JS, so try static pages first,
    # then browser pages under jobs.jobvite.com.
    name = company["name"]
    parsed = urlparse(source_url)
    jobvite_company = company.get("jobvite_company") or "arteris"
    candidates = []
    candidates.extend(get_urls(company))
    candidates.extend([
        f"https://jobs.jobvite.com/{jobvite_company}/search",
        f"https://jobs.jobvite.com/{jobvite_company}/jobs",
    ])
    jobs: dict[str, Job] = {}
    for url in dict.fromkeys(candidates):
        try:
            r = fetch(url)
            soup = BeautifulSoup(r.text, "html.parser")
            for j in jobs_from_soup(name, url, soup, meta="jobvite-static"):
                if "jobs.jobvite.com" in j.url or "/job/" in j.url:
                    jobs[j.id] = j
        except Exception:
            pass
    if not jobs:
        for url in dict.fromkeys(candidates):
            try:
                for j in parse_browser({**company, "name": name, "adapter": "browser"}, url):
                    if "jobvite" in j.url or "/job/" in j.url:
                        jobs[j.id] = Job(j.id, j.company, j.title, j.url, j.location, "jobvite-browser")
            except Exception:
                pass
    return list(jobs.values())
...
def parse_workday(company: dict[str, Any], source_url: str) -> list[Job]:
    ...
    while True:
        ...
        if total is not None and offset >= int(total):
            break
        if offset >= int(company.get("max_api_jobs", 250)):
            break
    return list(jobs.values())


def iter_dicts(obj: Any, max_nodes: int = 600) -> Iterable[dict[str, Any]]:
    stack = [obj]
    seen = 0
...
def eightfold_candidate_to_job(company_name: str, base: str, domain: str, p: dict[str, Any], source_url: str) -> Job | None:
    title = normalize_space(
        p.get("name") or p.get("title") or p.get("position_name") or p.get("position") or p.get("displayName") or ""
    )
    if is_bad_title(title) or "talent community" in title.lower() or "rejoignez notre communaut" in title.lower():
        return None
    job_id = str(p.get("id") or p.get("pid") or p.get("position_id") or p.get("ats_job_id") or p.get("requisition_id") or "")
    job_url = p.get("canonicalPositionUrl") or p.get("url") or p.get("apply_url") or p.get("position_url") or p.get("external_url")
    if not job_url and job_id:
        job_url = f"{base}/careers/job/{job_id}?domain={domain}"
    if not job_url:
        job_url = source_url
    job_url = urljoin(base, str(job_url))
    loc = p.get("location") or p.get("locations") or p.get("location_display") or p.get("locationDisplay") or ""
    if isinstance(loc, list):
        location_text = " · ".join(normalize_space(x.get("name") if isinstance(x, dict) else str(x)) for x in loc[:4])
    elif isinstance(loc, dict):
        location_text = normalize_space(" ".join(str(v) for v in loc.values() if v))
    else:
        location_text = normalize_space(str(loc))
    jid = str(job_id or stable_id(company_name, title, job_url))
    return Job(id=jid, company=company_name, title=title, url=job_url, location=location_text, meta="eightfold-api")
...
def parse_eightfold(company: dict[str, Any], source_url: str) -> list[Job]:
    ...
    # Eightfold has at least two public career-site patterns: SmartApply and PCSX.
    attempts: list[tuple[str, str, dict[str, Any] | None]] = []
    for start in range(0, int(company.get("max_api_jobs", 200)), 50):
        params = {"domain": domain, "query": query, "location": location, "sort_by": sort_by, "num": "50", "start": str(start)}
        attempts.append(("GET", f"{base}/api/apply/v2/jobs?{urlencode(params)}", None))
        attempts.append(("GET", f"{base}/api/pcsx/search?{urlencode(params)}", None))
        body = {"domain": domain, "query": query, "location": location, "sort_by": sort_by, "num": 50, "start": start}
        attempts.append(("POST", f"{base}/api/pcsx/search", body))

    for method, url, body in attempts:
        try:
            r = fetch(url, method=method, json_body=body, extra_headers={"Accept": "application/json", "Content-Type": "application/json"})
            data = r.json()
        except Exception:
            continue
        before = len(jobs)
        for p in iter_dicts(data):
            # Real position records usually have a title/name and an ID/pid or URL.
            if not any(k in p for k in ("name", "title", "position_name", "position", "displayName")):
                continue
            if not any(k in p for k in ("id", "pid", "position_id", "ats_job_id", "requisition_id", "canonicalPositionUrl", "position_url", "apply_url")):
                continue
            job = eightfold_candidate_to_job(name, base, domain, p, source_url)
            if job:
                jobs[job.id] = job
        # Stop only after we have jobs and this page added no new data after a non-zero start.
        if len(jobs) == before and jobs and (body or "start=0" not in url):
            break
    return list(jobs.values())
...
def browser_extract_one(company: dict[str, Any], source_url: str) -> list[Job]:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for label in ["Accept All", "Accept all", "Tout accepter", "Accepter", "I agree", "Agree", "OK", "Ok"]:
                try:
            ...
            for _ in range(MAX_BROWSER_CLICKS):
                clicked = False
                buttons = page.locator("button, a[role='button'], [role='button'], a[href*='show_more']")
                count = min(buttons.count(), 120)
                for i in range(count):
                    try:
                        b = buttons.nth(i)
                        txt = normalize_space(b.inner_text(timeout=700))
                        if txt and CLICK_BUTTON_RE.search(txt) and b.is_visible():
                            b.click(timeout=3500)
                ...
                () => {
                  ...
                  for (const a of Array.from(document.querySelectorAll('a[href]'))) {
                    ...
                    for (let depth = 0; n && depth < 6; depth++, n = n.parentElement) {
                      ...
                      const prefix = t.slice(0, 240).toLowerCase();
    ...
    for item in raw_items:
        ...
        add_job(jobs, company_name, text, href, loc, "browser")
    return list(jobs.values())
...
def parse_one_url(company: dict[str, Any], source_url: str) -> list[Job]:
    ...
    if adapter == "apple" or "jobs.apple.com" in host:
        return parse_apple(company, source_url)
    if adapter == "secure_ic" or "secure-ic.fr" in host or "secure-ic.com" in host:
        return parse_secure_ic(company, source_url)
    if adapter == "icalps" or "icalps.com" in host:
        return parse_icalps(company, source_url)
    if adapter == "menta" or "menta-efpga.com" in host:
        return parse_menta(company, source_url)
    if adapter == "scalinx" or "scalinx.com" in host:
        return parse_scalinx(company, source_url)
    if adapter == "teamtailor" or host in {"careers.ingroupe.com", "careers.dolphin-semiconductor.com", "careers.sipearl.com"}:
        return parse_teamtailor(company, source_url)
    if adapter == "jobvite" or "jobs.jobvite.com" in host or "arteris.com" in host:
        return parse_jobvite(company, source_url)
...
def send_snapshot(parsed: dict[str, list[Job]], mode: str, health_warnings: list[str] | None = None) -> None:
    lines: list[str] = []
    ...
    if health_warnings:
        lines.append("\n⚠️ Count warnings:")
        lines.extend(f"• {w}" for w in health_warnings)
...
def count_warning_for(company: dict[str, Any], jobs: list[Job]) -> str | None:
    minimum = company.get("min_expected_jobs")
    if minimum is None:
        return None
    try:
        minimum = int(minimum)
        ...
        return None
    if len(jobs) < minimum:
        return f"{company['name']}: only {len(jobs)} detected, expected at least {minimum}. Parser/link may need attention."
    return None
...
def main() -> int:
    ...
    for company in config["companies"]:
        name = company["name"]
        cstate = state["companies"].setdefault(name, {"seen_ids": [], "last_count": None, "last_error": None, "last_warning": None})
        try:
            jobs = parse_company(company)
            parsed_by_company[name] = jobs
            warn = count_warning_for(company, jobs)
            if warn:
                health_warnings.append(warn)
                if initialized and NOTIFY_MODE == "new" and cstate.get("last_warning") != warn:
                    parser_alerts.append(warn)
                cstate["last_warning"] = warn
            else:
                cstate["last_warning"] = None
    ...
    if NOTIFY_MODE in {"sample", "all"}:
        send_snapshot(parsed_by_company, NOTIFY_MODE, health_warnings)
        if errors:
    ...
    if NOTIFY_MODE == "reset_baseline":
        ...
        for company in config["companies"]:
            company_name = company["name"]
            jobs = parsed_by_company.get(company_name, [])
            warn = count_warning_for(company, jobs)
        ...
        if health_warnings:
            tg_send("⚠️ Count warnings after baseline reset:\n\n" + "\n".join(health_warnings))
        if errors:
    ...
    if parser_alerts:
        tg_send("⚠️ Job monitor parser/count problem(s)\n\n" + "\n".join(parser_alerts))
